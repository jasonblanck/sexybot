...
class PolymarketBot:
    ...
    # SPY (S&P 500 ETF) + ^DJI (Dow index) + ^IXIC (Nasdaq Composite)
    # feed the dashboard's INDEXES panel. ETFs DIA/QQQ would be more
    # natural matches for SPY's slot but they're 402 Payment Required
    ...
    def _gamma_market_outcome(self, token_id: str) -> Optional[int]:
        ...
        try:
            # Query the markets endpoint by clob_token_ids (omit closed=true so newly resolved or active markets aren't excluded)
    ...
    async def analyze_with_claude(self, market: dict, yes_p: float, research: dict) -> Optional[dict]:
        ...
        try:
            ...
            system_prompt = (
                ...
                "Example E — sports moneyline at YES=0.93, the favorite "
                "is up 14 with 3 minutes left and the loser is fouling: "
                "probability ~96, confidence 60, SKIP. The remaining "
                "edge (~3 cents on a near-certainty) is below the noise "
                "floor and the upside (~7% if right, total loss if wrong) "
                "is asymmetric in the wrong direction.\n"
                "Example F — long-dated political market at YES=0.18 with "
                "a 9-month resolution window, no immediate catalyst, OBI "
                "+0.05: probability ~22, confidence 40, SKIP. Long horizons "
                "decay edge into noise; even a real 4-point edge can be "
                "wiped out by rebalancing flow before resolution. Save "
                "Kelly for closer markets.\n"
                "Example G — weather market at YES=0.55 (rain by date), "
                "forecast model shows 70% probability, OBI +0.30, no "
                "conflicting signals: probability ~70, confidence 75, BUY "
                "YES, risk medium. Weather forecasts are well-calibrated, "
                "OBI confirms, edge is meaningful.\n"
                "\n"
                "## Common mistakes to actively recognize\n"
                "1. Treating the market price as your prior. The market "
                "is one signal among several; you should form an "
                "independent probability and only then compare. If you "
                "find yourself adjusting *toward* the market price as "
                "you reason, you're anchoring.\n"
                "2. Letting a single recent headline dominate the "
                "estimate. One news item rarely justifies a >10-point "
                "departure from the market unless it is genuinely "
                "decision-relevant (court ruling, official announcement, "
                "settled outcome). Vibes-based news adjustments are noise.\n"
                "3. Picking BUY because you have an opinion. The action "
                "default is SKIP. BUY requires (a) meaningful edge, "
                "(b) supportive order-book pressure or at least neutral, "
                "(c) confidence proportional to the data quality. If any "
                "of these fail, SKIP is correct even if your view is "
                "directionally right.\n"
                "4. Confusing high confidence with high edge. A 95% "
                "confidence reading on a 1-point edge is still SKIP — "
                "the small edge is below execution friction. A 50% "
                "confidence reading on a 10-point edge is also SKIP — "
                "you don't trust the estimate enough to act.\n"
                "5. Forgetting transaction costs. Polymarket's 1-2 cent "
                "spread eats most of the edge on near-fair markets. Your "
                "edge calculation should net spread; if it doesn't, you "
