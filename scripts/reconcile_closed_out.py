...
DB_PATH = 'trades' + '.db'  # standard name

def main(argv=None):
    import argparse
    parser = argparse.ArgumentParser(description="Reconcile closed out round-trip trades.")
    parser.add_argument("--dry-run", action="store_true", help="Reconcile dry-run trades instead of live trades")
    parser.add_argument("--all", action="store_true", help="Reconcile both live and dry-run trades")
    ...
    if args.all:
        ...
        dry_run_clause = "AND dry_run = 1"
    else:
        dry_run_clause = "AND (dry_run IS NULL OR dry_run = 0)"

    # Query successful trades
    print("Querying valid trades...")
    trades = cursor.execute(f"""
        SELECT id, token_id, side, price, shares, amount, resolved, realized_pnl, time, market, dry_run
        FROM trades
    ...
    # Group trades by (token_id, dry_run)
    token_trades = defaultdict(list)
    for t in trades:
        token_trades[(t['token_id'], t['dry_run'])].append(t)

    # Filter for groups that have both BUY and SELL trades
    tokens_to_reconcile = {}
    for key, t_list in token_trades.items():
        has_buy = any(t['side'].upper() == 'BUY' for t in t_list)
        has_sell = any(t['side'].upper() == 'SELL' for t in t_list)
        if has_buy and has_sell:
            tokens_to_reconcile[key] = t_list

    print(f"Found {len(tokens_to_reconcile)} unique groups with round-trip trades.")
    ...
    for idx, ((token_id, dry_run), t_list) in enumerate(tokens_to_reconcile.items(), 1):
        market_name = t_list[0]['market']
        run_label = "Dry-Run" if dry_run == 1 else "Live"
        ...
        # Apply updates for this token ID group
        for buy_id, matched_pnl in buy_trades_to_update.items():
            ...
            cursor.execute("""
                UPDATE trades
                SET resolved = 1, realized_pnl = ?, resolved_at = ?, won = ?
                WHERE id = ?
            """, (round(matched_pnl, 4), latest_time, 1 if matched_pnl > 0 else 0, buy_id))
            ...
            if old_pnl is not None:
                ...
                if abs(pnl_diff) > 0.01:
                    print(f"  [{run_label} PnL] Buy ID {buy_id} on '{market_name[:40]}': ${old_pnl:+.2f} -> ${matched_pnl:+.2f} (diff ${pnl_diff:+.2f})")
                    total_realized_pnl_change += pnl_diff
            else:
                print(f"  [{run_label} Settle] Buy ID {buy_id} on '{market_name[:40]}': realized PnL = ${matched_pnl:+.2f}")
    ...
    # Print total resolved trades summaries
    if args.all:
        for dr in (0, 1):
            label = "Dry-Run" if dr == 1 else "Live"
            pnl_summary = cursor.execute(f"""
                SELECT COUNT(*), SUM(realized_pnl), 
                       SUM(CASE WHEN realized_pnl > 0 THEN 1 ELSE 0 END)
                FROM trades
                WHERE resolved=1 AND dry_run={dr}
            """).fetchone()
            if pnl_summary and pnl_summary[0] > 0:
                cnt, total_pnl, wins = pnl_summary
                total_pnl = total_pnl or 0.0
                win_rate = (wins / cnt * 100) if cnt > 0 else 0.0
                print(f"Current {label} database summary (resolved=1):")
        ...
        pnl_summary = cursor.execute(f"""
            SELECT COUNT(*), SUM(realized_pnl), 
                   SUM(CASE WHEN realized_pnl > 0 THEN 1 ELSE 0 END)
            FROM trades
            WHERE resolved=1 AND {dry_run_clause.replace("AND ", "")}
        """).fetchone()
        if pnl_summary and pnl_summary[0] > 0:
            cnt, total_pnl, wins = pnl_summary
            total_pnl = total_pnl or 0.0
