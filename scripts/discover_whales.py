...
def main() -> None:
    ...
    if diversified:
        ...
        staging_pool = diversified
        staging_label = "diversified active"
    else:
        print("  (none in this sample — try DISCOVER_PAGES=10 for a bigger sample)")
        print("  Falling back to raw top-by-volume for the staging file.")
        staging_pool = ranked
        staging_label = "raw top-by-volume (no diversification filter passed)"

    # Write staging file for operator review
    if WRITE_STAGING and staging_pool:
        try:
            wallets_csv = ",".join(w for w, _ in staging_pool[:TOP_N])
            with open(STAGING_FILE, "w") as f:
                f.write(f"# Suggested whale wallets — generated by discover_whales.py at {os.popen('date -u').read().strip()}\n")
                f.write(f"# Top {len(staging_pool[:TOP_N])} {staging_label} by recent volume.\n")
